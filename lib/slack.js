...
module.exports = function* (body) {
  var uri = process.env.GITHUB_SLACK_POST_URL || "";
...
function fmtSlack(event) {
  ...
  switch (event.type) {
    case "PullRequestEvent":
      if (event.payload.action == "closed") {
        text_fmt =
          "Merged/Closed pull request " + event.payload.pull_request.html_url;
      } else {
    ...
    case "CreateEvent":
      if (!event.payload.ref) {
        ...
        text_fmt =
          "Created branch " +
          friendly(event.repo.url + "/tree/" + event.payload.ref);
      }
      break;
    case "IssuesEvent":
    ...
    case "IssueCommentEvent":
      text_fmt =
        ":speech_balloon: Commented on issue " +
    ...
    case "DeleteEvent":
      text_fmt =
        ...
        "`" +
        " from " +
        friendly(event.repo.url);
      break;
    case "PushEvent":
      var branch = event.payload.ref.substr(
        event.payload.ref.lastIndexOf("/") + 1,
      );
      var commit_url =
        "https://github.com" +
        "/" +
    ...
    case "ForkEvent":
      ...
      break;
    case "PublicEvent":
      text_fmt = ":book: Made " + friendly(event.repo.url) + " public";
      break;
    case "PullRequestReviewCommentEvent":
      text_fmt =
        ":speech_balloon: Commented on pull request " +
        event.payload.comment.html_url +
        "\n\r" +
        "> " +
        event.payload.comment.body;
  ...
  return JSON.stringify({
    ...
    icon_url:
      ...
      "https://raw.githubusercontent.com/lambtron/github-social-slack/master/img/github-slack.png",
  });
}
